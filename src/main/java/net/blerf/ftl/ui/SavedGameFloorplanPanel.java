...
public class SavedGameFloorplanPanel extends JPanel {
    ...
    private void showSystemEditor(final ShipBundle shipBundle, final SpriteReference<SystemState> systemRef) {
        ...
        int currentReserveCapacity = shipBundle.getReservePowerCapacity();
        int minReserveCapacity = ftlConstants.getMaxReservePoolCapacity() - excludedReservePool;
        // S'assurer que le minimum ne dépasse pas la valeur actuelle
        minReserveCapacity = Math.min(minReserveCapacity, currentReserveCapacity);
        editorPanel.getSlider(RESERVE_CAPACITY).setMinimum(minReserveCapacity);
        editorPanel.getSlider(RESERVE_CAPACITY).setValue(currentReserveCapacity);
        ...
        final Runnable applyCallback = new Runnable() {
            @Override
            public void run() {
                int newReserveCapacity = editorPanel.getSlider(RESERVE_CAPACITY).getValue();
                if (newReserveCapacity != shipBundle.getReservePowerCapacity()) {
                    shipBundle.setReservePowerCapacity(newReserveCapacity);
                }
